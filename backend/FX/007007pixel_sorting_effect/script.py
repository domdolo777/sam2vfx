...
def apply_effect(frame, **params):
    ...
    # Apply pixel sorting
    sorted_image, mask = pixel_sort(resized_frame, sorting_direction, sorting_style, thresh_min, thresh_max)

    # Upscale back to original resolution
    output_frame = cv2.resize(sorted_image, (w, h))
    mask = cv2.resize(mask.astype(np.uint8) * 255, (w, h), interpolation=cv2.INTER_NEAREST)
    mask = mask.astype(bool)

    # Blend the sorted output with the original frame
    blended_image = frame.copy()
    blended_image[mask] = output_frame[mask]

    # Add noise if required
    if noise_amount > 0:
        noise = np.random.normal(0, noise_scale, output_frame.shape).astype(np.uint8)
        blended_image = cv2.addWeighted(blended_image, 1 - (noise_amount / 100), noise, noise_amount / 100, 0)
