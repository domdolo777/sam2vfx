...
def apply_effect(frame, mask=None, **params):
    ...
    # Ensure that areas outside the mask remain unaffected
    final_output = frame.copy()  # Start with the original frame
    final_output[mask] = cv2.addWeighted(frame[mask], 1 - opacity, output_frame[mask], opacity, 0)
