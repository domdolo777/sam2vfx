...
def apply_effect(frame, **params):
    ...
    sorting_intensity = get_param(params, "sorting_intensity", 1, float, 0.1, 10.0)  # New parameter for sorting intensity

    # Adjust contrast and saturation
    frame = cv2.convertScaleAbs(frame, alpha=contrast, beta=0)
    hsv_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    hsv_frame[:, :, 1] = np.clip(hsv_frame[:, :, 1] * saturation, 0, 255).astype(np.uint8)
    frame = cv2.cvtColor(hsv_frame, cv2.COLOR_HSV2BGR)

    # Apply sorting logic based on threshold
    def pixel_sort(image, axis, style, thresh_min, thresh_max, intensity):
    ...
    # Apply pixel sorting
    sorted_image = pixel_sort(resized_frame, sorting_direction, sorting_style, thresh_min, thresh_max, sorting_intensity)

    # Apply noise if needed
    if noise_amount > 0:
        noise = np.random.normal(0, noise_scale, sorted_image.shape).astype(np.float32)
        sorted_image = cv2.addWeighted(sorted_image.astype(np.float32), 1 - (noise_amount / 100), noise, noise_amount / 100, 0).astype(np.uint8)
