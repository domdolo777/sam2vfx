...
def apply_effect(frame, mask=None, **params):
    ...
    # Extract parameters
    threshold_min = get_param(params, "threshold_min", 0, float, 0, 255)
    threshold_max = get_param(params, "threshold_max", 255, float, 0, 255)
    ...
    expand = get_param(params, "expand", 0, int, -50, 50)

    # Apply rotation if necessary
    if angle != 0:
        frame = rotate_image(frame, angle)

    # Apply contrast and saturation properly
    ...
    def pixel_sort(image, mask, axis, style, thresh_min, thresh_max):
        ...
        # Ensure thresholds are handled properly and smoothly
