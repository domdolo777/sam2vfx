...
def apply_effect(frame, mask=None, **params):
    ...
    def pixel_sort(image, mask, axis, style, thresh_min, thresh_max):
        ...
        # Enforce mask: sorting only happens within the masked area
        pixel_mask = mask  # STRICT mask enforcement
        pixel_mask = (channel_to_sort >= thresh_min) & (channel_to_sort <= thresh_max) & pixel_mask

        # Sort pixels within the mask along the specified axis
