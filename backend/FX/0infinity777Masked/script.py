...
def apply_effect(frame, mask=None, **params):
    ...
    # Add noise if required (ensure type compatibility)
    if noise_amount > 0:
        noise = np.random.normal(0, noise_scale, output_frame.shape).astype(np.float32)
        output_frame = cv2.addWeighted(output_frame.astype(np.float32), 1 - (noise_amount / 100), noise, noise_amount / 100, 0)

    # Convert output to uint8 for consistent output
