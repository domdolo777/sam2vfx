...
def apply_effect(frame, **params):
    ...
    # Example segmentation mask (you need to provide the actual mask in practice)
    ...
    def pixel_sort(image, mask, axis, style, thresh_min, thresh_max):
        ...
        if axis == 0:  # Vertical sorting
            for col in range(image.shape[1]):
                masked_indices = np.where(mask_indices[:, col])[0]
            ...
            for row in range(image.shape[0]):
                masked_indices = np.where(mask_indices[row, :])[0]
                if len(masked_indices) > 0:
