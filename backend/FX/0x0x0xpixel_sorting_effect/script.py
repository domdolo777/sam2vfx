    ...
    def pixel_sort(image, axis, style, thresh_min, thresh_max):
        ...
        if axis == 0:  # Vertical sorting
            for col in range(image.shape[1]):
                # Extract pixels along a column that match the mask condition
                column_pixels = image[:, col, :][mask[:, col]]
                if len(column_pixels) > 0:
                    # Sort pixels within the threshold range
                    sorted_pixels = np.sort(column_pixels, axis=0)
                    # Stretch or fill to cover the entire column
                    sorted_image[:, col, :] = np.tile(sorted_pixels, (len(sorted_image[:, col]) // len(sorted_pixels), 1))[:len(sorted_image[:, col])]
        else:  # Horizontal sorting
            for row in range(image.shape[0]):
                # Extract pixels along a row that match the mask condition
                row_pixels = image[row, :, :][mask[row, :]]
                if len(row_pixels) > 0:
                    # Sort pixels within the threshold range
                    sorted_pixels = np.sort(row_pixels, axis=0)
                    # Stretch or fill to cover the entire row
                    sorted_image[row, :, :] = np.tile(sorted_pixels, (len(sorted_image[row]) // len(sorted_pixels), 1))[:len(sorted_image[row])]
