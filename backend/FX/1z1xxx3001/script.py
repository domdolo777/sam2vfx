...
def apply_effect(frame, mask=None, **params):
    ...
    direction = get_param(params, "direction", "vertical", str)  # New param for direction (vertical/horizontal)
    ...
    def pixel_sort(image, mask, direction, style, thresh_min, thresh_max, strip_width, strip_spacing, pulling_distance):
        ...
        # Depending on the direction (vertical or horizontal) apply the sorting
        if direction == "horizontal":
            ...
            for row in range(0, image.shape[0], strip_width + strip_spacing):
                ...
                if len(masked_indices) > 1:
                    for i in range(0, len(masked_indices), strip_width):
                        # Pull and sort pixels horizontally within strip width
                        block_indices = masked_indices[i:i + strip_width]
                        pulling_indices = np.clip(block_indices * pulling_distance, 0, image.shape[1] - 1).astype(int)
                        sorted_indices = np.argsort(channel_to_sort[row, block_indices])
                        sorted_image[row, pulling_indices] = image[row, block_indices[sorted_indices]]
        else:
            ...
            for col in range(0, image.shape[1], strip_width + strip_spacing):
                ...
                if len(masked_indices) > 1:
                    for i in range(0, len(masked_indices), strip_width):
                        # Pull and sort pixels vertically within strip width
                        block_indices = masked_indices[i:i + strip_width]
                        pulling_indices = np.clip(block_indices * pulling_distance, 0, image.shape[0] - 1).astype(int)
                        sorted_indices = np.argsort(channel_to_sort[block_indices, col])
                        sorted_image[pulling_indices, col] = image[block_indices[sorted_indices], col]

        # Apply mask so sorting only happens inside the masked region
        if mask is not None:
            sorted_image = np.where(mask[..., None] > 0, sorted_image, image)

        return sorted_image

    # Apply pixel sorting based on the specified direction
    sorted_image = pixel_sort(frame, mask, direction, sorting_style, thresh_min, thresh_max, strip_width, strip_spacing, pulling_distance)
