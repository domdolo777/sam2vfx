...
# Function to apply pixel sorting within the masked region only
def apply_effect(frame, mask=None, **params):
    ...
    if mask is None:
        mask = np.ones_like(frame[:, :, 0], dtype=np.uint8)  # Default to a full mask if none provided
    else:
        mask = mask.astype(np.uint8)  # Ensure mask is of the correct type

    # Core pixel sorting logic, applied only within the mask region
    def pixel_sort(image, direction, style, thresh_min, thresh_max, strip_width, strip_spacing, pulling_distance):
        ...
        # Threshold mask for sorting (applied within the given mask)
        threshold_mask = ((channel_to_sort >= min_thresh_value) & 
                          (channel_to_sort <= max_thresh_value) &
                          (mask > 0))  # Only sort within the mask
