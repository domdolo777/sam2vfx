...
def apply_effect(frame, **params):
    ...
    # Ensure correct frame data type
    if frame.dtype == np.float64:  # If the frame is in 64-bit float, convert it to float32
        frame = frame.astype(np.float32)
    elif frame.dtype != np.float32:  # Otherwise, if it's not float32, convert it to float32
    ...
    # Convert frame back to uint8 if necessary
