...
def apply_effect(frame, **params):
    ...
    # Adjust angle for pixel sorting
    ...
    def pixel_sort(image, style, thresh_min, thresh_max, strip_width, strip_spacing, pulling_distance):
        ...
        # Convert threshold values to the actual range of the channel being sorted
        if style in [0, 1]:  # For Hue and Saturation, the range is 0-255
            min_thresh_value = int(thresh_min * 255)
            max_thresh_value = int(thresh_max * 255)
        else:  # For Lightness (grayscale), the range is also 0-255
            min_thresh_value = int(thresh_min * 255)
            max_thresh_value = int(thresh_max * 255)

        # Mask for threshold range
        mask = (channel_to_sort >= min_thresh_value) & (channel_to_sort <= max_thresh_value)
