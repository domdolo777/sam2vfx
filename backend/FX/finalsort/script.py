...
def apply_effect(frame, mask=None, **params):
    ...
    # Ensure mask is applied or generated as needed
    if mask is None:
        mask = np.ones(frame.shape[:2], dtype=bool)  # Full mask if not provided
    ...
    def pixel_sort(image, mask, style, thresh_min, thresh_max, strip_width, strip_spacing, pulling_distance):
        ...
        # Create mask based on threshold range and apply to the pixel-sorting mask
