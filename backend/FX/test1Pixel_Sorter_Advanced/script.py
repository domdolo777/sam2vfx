...
def apply_effect(frame, mask=None, **params):  # Set mask=None as default
    if not params:
        params = {}
    ...
    angle = get_param(params, "angle", 0, float, -360.0, 360.0)  # Angle can be negative for inversion
    ...
    def pixel_sort(image, mask, style, thresh_min, thresh_max, strip_width, strip_spacing, pulling_distance):
        ...
        # Ensure mask is properly initialized
        if mask is None:
            mask = np.ones(channel_to_sort.shape, dtype=bool)  # Use full frame if mask is not provided
        else:
