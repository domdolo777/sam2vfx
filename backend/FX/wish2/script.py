...
def apply_effect(frame, sam2_mask, **params):
    if not params:
        params = {}
    ...
    # Ensure the SAM2 mask is binary (0 or 255)
    ...
    sorted_image = pixel_sort(frame, sam2_mask, sorting_direction, sorting_style, thresh_min, thresh_max)
    ...
    final_image = cv2.add(final_image, cv2.bitwise_and(sorted_image, sorted_image, mask=sam2_mask))
