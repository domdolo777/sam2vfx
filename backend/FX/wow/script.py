...
def apply_effect(frame: np.ndarray, **params) -> np.ndarray:
    ...
    Parameters:
        ...
        sort_in (bool): Enables pixel sorting from start to end.
        sort_out (bool): Enables pixel sorting from end to start.
    ...
    # Adjust frame contrast and saturation for better visibility
    frame = cv2.convertScaleAbs(frame, alpha=1.0, beta=0)  # You can tweak alpha if needed
    hsv_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    hsv_frame[:, :, 1] = np.clip(hsv_frame[:, :, 1] * 1.2, 0, 255).astype(np.uint8)  # Increase saturation by 20%
    ...
    def pixel_sort(image: np.ndarray, axis: int, style: int, thresh_min: int, thresh_max: int, sort_in: bool, sort_out: bool, sort_size: int, intensity: float) -> np.ndarray:
        ...
        for i in iterator:
            if axis == 0:  # Vertical sorting
                window_start = max(i - sort_size, 0)
                window_end = min(i + sort_size + 1, image.shape[0])
                ...
                if len(masked_indices) > 1:
                    ...
                    sorted_pixels = image[window, masked_indices[sorted_indices]].astype(np.float32) * intensity
                    sorted_pixels = np.clip(sorted_pixels, 0, 255).astype(np.uint8)
                    sorted_image[window, masked_indices] = sorted_pixels
            else:  # Horizontal sorting
                window_start = max(i - sort_size, 0)
                window_end = min(i + sort_size + 1, image.shape[1])
                window = slice(window_start, window_end)
                masked_indices = np.where(mask[:, window])[0]
                if len(masked_indices) > 1:
                    sorted_indices = np.argsort(channel_to_sort[masked_indices, window])
                    sorted_pixels = image[masked_indices, window][sorted_indices].astype(np.float32) * intensity
                    sorted_pixels = np.clip(sorted_pixels, 0, 255).astype(np.uint8)
                    sorted_image[masked_indices, window] = sorted_pixels

        return sorted_image

    # Apply pixel sorting with advanced options
    sorted_frame = pixel_sort(
        frame,
        sorting_direction,
        sorting_style,
        thresh_min,
